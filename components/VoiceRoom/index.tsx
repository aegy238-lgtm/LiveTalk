    ...
    try {
      ...
      recIds.forEach(rid => {
        ...
        if (targetUser?.isHost && targetUser.hostAgencyId) {
           // سيستم الوكالة: المضيف 70%، الوكيل 1%
           ...
           // جلب الوكيل لإضافة العمولة
           getDoc(doc(db, 'host_agencies', targetUser.hostAgencyId)).then(agencySnap => {
              if (agencySnap.exists()) {
                 const agencyData = agencySnap.data();
                 updateDoc(doc(db, 'users', agencyData.agentId), {
                    diamonds: increment(agentCommission)
                 });
                 updateDoc(doc(db, 'host_agencies', targetUser.hostAgencyId), {
                    totalProduction: increment(cost)
                 });
              }
           });
        } else {
           // النظام العادي للمستخدمين غير المنضمين لوكالة
           batch.update(doc(db, 'users', rid), {
             charm: increment(cost / recIds.length),
